...
from proton.vpn.app.gtk.widgets.headerbar.menu.settings.split_tunneling.app.data_structures \
    import AppData, AppRowWithRemoveButton
...
def test_remove_app_is_removed_and_signal_is_emitted_when_app_is_removed_from_list(mock_app_native):
    mock_app_removed_callback = Mock()
    sap = SelectedAppList(apps_to_add=[mock_app_native])
...
def test_refresh_apps_are_added_to_list_when_they_are_newly_selected(mock_app_native):
    ...
    sap.refresh(selected_apps=[mock_app_native])

    process_gtk_events()

    mock_app_list_refreshed_callback.assert_called_once()
    assert mock_app_list_refreshed_callback.call_args_list[0][0][1] == [mock_app_native]


def test_refresh_apps_are_removed_from_list_when_they_are_deselected(mock_app_native):
    mock_app_list_refreshed_callback = Mock()
...
def test_refresh_apps_list_is_not_updated_when_apps_are_untouched(mock_app_native):
    mock_app_list_refreshed_callback = Mock()
